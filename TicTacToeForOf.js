 ...
 *
 *
 * Okay, funktioniert so. Aber ist irgendwie uncool.
 * Das Befüllen geht doch bestimmt auch mit Schleifen.
 * Aber egal wie, definiert werden muss das Array so oder so.
 */
let ticTac = [[], [], []]

//Erstes Befüllen von ticTac

//Schleife zum Durchlaufen des äußeren Arrays (der "Zeilen")
for (let zeile = 0; zeile < ticTac.length; zeile++) {
  //Schleife zum Durchlaufen des jeweiligen inneren Arrays (der "Spalten")
  for (let spalte = 0; spalte < ticTac.length; spalte++) {
    ticTac[zeile][spalte] = "_"
  }
}
 ...
 *
 * Auch das lässt sich doch bestimmt mit einer Schleife hübscher machen?!?
 *
 */

let ausgabe = ""

for (let zeile of ticTac) {
  ausgabe = ausgabe + zeile.join(" ") + "\n"
}
